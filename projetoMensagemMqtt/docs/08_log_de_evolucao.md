...
|---|---|---|---|---|
| A barra de navegação do Android cobria o input do Chat e o conteúdo encostava na status bar (safe area "não funcionava") | `App.tsx` envolvia tudo em `SafeAreaView edges={['top']}`; com o edge-to-edge obrigatório no Expo SDK 54 o inset inferior nunca era aplicado e o topo ficava branco, destoando do header | Humano (screenshot) | Front-end | `useSafeAreaInsets` por tela: header com `insets.top`, input/FAB/conteúdo com `insets.bottom`; `tsc` exit 0; validado nas 3 telas no emulador Android (3 botões) |
